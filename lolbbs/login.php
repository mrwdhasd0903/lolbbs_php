...
if(isset($_POST['submit'])){
	...
	escape($link,$_POST);
	$query="select * from lol_member where name='{$_POST['name']}' and pw=md5('{$_POST['pw']}')";
	$result=execute($link, $query);
	if(mysqli_num_rows($result)==1){
		setcookie('lol[name]',$_POST['name'],time()+$_POST['time']);
		setcookie('lol[pw]',sha1(md5($_POST['pw'])),time()+$_POST['time']);
		...
		<form method="post">
			...
			<label>自动登录：
				<select style="width:236px;height:25px;" name="time">
					<option value="3600">1小时内（未完成）</option>
