...
if(!$member_id=is_login($link)){
	skip('login.php', 'error', '还未登陆！');
}
$query="select * from lol_member where id={$member_id}";
...
if(isset($_POST['submit'])){
	...
	if($upload['return']){
		$query="update lol_member set photo='{$upload['save_path']}' where id={$member_id}";
		execute($link, $query);
...
<style type="text/css">
body {
	background: url(style/bg.jpg) top/cover no-repeat fixed;
	font-size:12px;
	font-family:微软雅黑;
}
h2 {
...
<body>
	<div id="main">
		...
		<div>
			<h3>原头像：</h3>
			<img style="width: 180px; height:180px;" src="<?php if($data_member['photo']!=''){echo SUB_URL.$data_member['photo'];}else{echo 'style/photo.jpg';}?>" />
			<br />
			最佳图片尺寸：180*180
		</div>
