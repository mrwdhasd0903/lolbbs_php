...
$query="select * from lol_member where id={$_GET['id']}";
...
$query="select count(*) from lol_content where member_id={$_GET['id']}";
...
<div id="main" class="auto">
	<div id="left">
		<ul class="postsList">
			...
			$query="select
			lol_content.title,lol_content.id,lol_content.time,lol_content.member_id,lol_content.times,lol_member.name,lol_member.photo
			from lol_content,lol_member where
			lol_content.member_id={$_GET['id']} and
			lol_content.member_id=lol_member.id order by id desc {$page['limit']}";
	...
	<div id="right">
		<div class="member_big">
			<dl>
				...
				<?php 
				//当查看的会员ID和登陆的id是同一个，才可以修改
				if($member_id==$data_member['id']){
				?>
				<dd  style="white-space:nowrap;" >操作：<a target="_blank" href="member_photo_update.php">修改头像</a> | <a target="_blank"href="">修改密码（未完成）</a></dd>
				<?php }?>
			</dl>
			<div style="clear:both;"></div>
		</div>
