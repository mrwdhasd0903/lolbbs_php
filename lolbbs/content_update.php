...
$query="select * from lol_content where id={$_GET['id']}";
...
if(mysqli_num_rows($result_content)==1){
	...
	if(check_user($member_id,$data_content['member_id'],$is_manage_login)){
		if(isset($_POST['submit'])){
			...
			$query="update lol_content set module_id={$_POST['module_id']},title='{$_POST['title']}',content='{$_POST['content']}' where id={$_GET['id']}";
			execute($link, $query);
			//如果是从板块跳过来则传个板块的地址，然后跳回去，否则一律跳回个人中心
