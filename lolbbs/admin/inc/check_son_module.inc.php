...
$query="select * from lol_father_module where id={$_POST['father_module_id']}";
...
switch ($check_flag){
	case 'add':
		$query="select * from lol_son_module where module_name='{$_POST['module_name']}'";
		break;
	case 'update':
		$query="select * from lol_son_module where module_name='{$_POST['module_name']}' and id!={$_GET['id']}";
		break;
	default:
...
if(mysqli_num_rows($result)){
	skip('son_module_add.php','error','子版块已存在！');
}
//学校机房的php默认配置用不了mb_strlen函数，所以注释了
