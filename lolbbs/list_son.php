...
//对比数据库 防止篡改
$query="select * from lol_son_module where id={$_GET['id']}";
...
$query="select count(*) from lol_content where module_id={$_GET['id']}";
$count_all=num($link,$query);
$query="select count(*) from lol_content where module_id={$_GET['id']} and time>CURDATE()";
$count_today=num($link,$query);
//查询的是版主的id，不是登陆用户的id
$query="select * from lol_member where id={$data_son['member_id']}";
	...
	<div id="left">
		<div class="box_wrap">
			...
			<div class="pages_wrap">
				<a class="btn publish" href="publish.php?son_module_id=<?php echo $_GET['id']?>" target="_blank">发帖</a>
				<div class="pages">
					<?php 
		...
		<ul class="postsList">
			...
			//多表查询  并且按照时间倒叙
			$query="select
			lol_content.title,lol_content.id,lol_content.time,lol_content.times,lol_content.member_id,lol_member.name,lol_member.photo
			from lol_content,lol_member where
			lol_content.module_id={$_GET['id']} and
			lol_content.member_id=lol_member.id ORDER BY lol_content.time DESC 
			{$page['limit']}";
