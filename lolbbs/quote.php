...
$query="select sc.id,sc.title,sm.name from lol_content sc,lol_member sm where sc.id={$_GET['id']} and sc.member_id=sm.id";
...
$query="select lol_reply.content,lol_member.name from lol_reply,lol_member where lol_reply.id={$_GET['reply_id']} and lol_reply.content_id={$_GET['id']} and lol_reply.member_id=lol_member.id";
...
if(isset($_POST['submit'])){
	...
	$query="insert into lol_reply(content_id,quote_id,content,time,member_id) 
			values(
...
$query="select count(*) from lol_reply where content_id={$_GET['id']} and id<={$_GET['reply_id']}";
